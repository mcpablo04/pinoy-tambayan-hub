  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Discover community-written Pinoy stories." />
      </Head>
        ...
        <div className="container-page max-w-5xl">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1 flex items-center gap-2">
                ...
                Latest drama and romance from the hub.
              </p>
            </div>
            <Link href="/stories/new" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-2xl font-bold transition-all hover:scale-105 shadow-lg shadow-blue-600/20">
              <PenTool size={18} />
              Write Yours
            </Link>
          </div>
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {loading ? (
              ...
              <div className="col-span-full py-20 text-center rounded-3xl border border-dashed border-white/10">
                <p className="text-gray-500">No stories found yet.</p>
              </div>
            ) : (
              stories.map((s) => (
                <li
                  ...
                  className="group cursor-pointer rounded-2xl border border-white/5 bg-white/[0.02] p-5 hover:bg-white/[0.05] transition-all flex flex-col justify-between"
                >
                  <div>
                  ...
                  <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4 text-gray-500 text-xs font-bold uppercase tracking-tighter">
                      <span className="flex items-center gap-1.5"><BookOpen size={14} /> {s.counts?.reads || 0}</span>
                      <span className="flex items-center gap-1.5"><MessageCircle size={14} /> {s.counts?.comments || 0}</span>
                    </div>
                  </div>
                </li>
              ))
          ...
          <div className="mt-12 flex items-center justify-center gap-6">
            <button
              ...
              className="px-5 py-2.5 rounded-xl border border-white/10 text-sm font-bold disabled:opacity-30 text-gray-300"
            >
              <ChevronLeft size={18} /> Prev
            </button>
            <span className="text-xs font-black text-blue-500 uppercase">Page {page + 1}</span>
            <button
